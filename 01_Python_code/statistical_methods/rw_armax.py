# 01_Python_code/statistical_methods/rw_armax.py
...
@dataclass
class RwArmaxRunResult:
    """
    Stores metadata for one rolling-window ARMAX run.
...
def select_best_armax_order(
    ...
    """
    Select the best ARMAX order using AIC.
    ...
    if best_order is None:
        raise RuntimeError("No ARMAX model fit succeeded for the current rolling window.")
...
def rw_armax_forecast(
    ...
    """
    Fit a rolling-window ARMAX model and generate one-step-ahead forecasts.
    ...
    output_folder = base_output_path / f"rw_armax_exog_{feature_nums_str}_{month}m_model_output"
    output_folder.mkdir(parents=True, exist_ok=True)

    result_file = output_folder / (
        f"rw_armax_exog_{feature_nums_str}_{month}m_model_results_{product}_{product_sign}_{price_type}.csv"
    )
    ...
    try:
        ...
        for i in range(len(test_df)):
            ...
            if len(train_raw) < 10 or len(exo_train) < 10:
                LOGGER.warning(
                    "Skipping %s because rolling train target/exog window is too short: y=%s, x=%s",
                    test_start_date,
                    len(train_raw),
                    len(exo_train),
                )
            ...
            try:
                ...
                LOGGER.warning(
                    "Skipping %s because no ARMAX order fit succeeded: %s",
                    test_start_date,
                    exc,
                )
                continue
            ...
            try:
                ...
                LOGGER.warning(
                    "Skipping %s because final ARMAX fit/forecast failed: %s",
                    test_start_date,
                    exc,
                )
                continue
...
def main() -> None:
    ...
    for month in months:
        feature_nums_str = "_".join(map(str, feature_nums))
        month_folder = BASE_OUTPUT_PATH / f"rw_armax_exog_{feature_nums_str}_{month}m_model_output"
        month_folder.mkdir(parents=True, exist_ok=True)

        month_overview = overview_df.loc[overview_df["WINDOW_MONTHS"] == month].copy()
        month_overview.to_csv(
            month_folder / f"rw_armax_exog_{feature_nums_str}_{month}m_models_overview.csv",
            index=False,
        )

        n_success = int(month_overview["SUCCESS"].sum()) if not month_overview.empty else 0
        LOGGER.info(
            "Finished rolling ARMAX runs for %sm: %s/%s successful",
            month,
            n_success,
            len(month_overview),
        )
...
# Example usage
# FEATURE_SET=1_3 python3 -m statistical_methods.rw_armax
