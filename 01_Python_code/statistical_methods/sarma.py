# 01_Python_code/statistical_methods/sarma.py

"""
SARMA forecasting script for aFRR capacity prices.

This script:
- loads the processed dataset
- fits SARMA models for each product / sign / price type combination
...
Notes:
- SARMA is implemented through statsmodels' SARIMAX API
- model order selection is done with pmdarima.auto_arima on the training set only
...
@dataclass
class SarmaRunResult:
    """
    Stores metadata for one SARMA run.

    Parameters:
        product: Time product identifier.
        product_sign: Regulation direction.
        price_type: Price type.
        order: Best non-seasonal order as a string.
        seasonal_order: Best seasonal order as a string.
        aic: Best AIC value.
...
def select_best_sarma_order(
    train_series: pd.Series,
    seasonal_period: int = 7,
) -> tuple[tuple[int, int, int], tuple[int, int, int, int], float]:
    """
    Select the best SARMA model using auto_arima on the training series.
    ...
    Raises:
        RuntimeError: If no valid SARMA model can be selected.
    """
    try:
...
def rolling_one_step_forecast_sarma(
    history: pd.Series,
    future: pd.Series,
    ...
    """
    Perform expanding-window one-step-ahead forecasting with model re-fitting.

    Parameters:
        history: Historical series available at the start.
        future: Future series to forecast one step at a time.
        order: Non-seasonal SARMA order.
        seasonal_order: Seasonal SARMA order.
...
def sarma_forecast(
    ...
    """
    Fit a SARMA model and generate validation/test forecasts for one configuration.
    ...
    Returns:
        SarmaRunResult with run metadata and status.
    """
    target_column = build_target_column(product_sign=product_sign, price_type=price_type)
    result_file = output_folder / f"sarma_model_results_{product}_{product_sign}_{price_type}.csv"
    ...
    try:
        ...
        if len(target_df) < 15:
            raise ValueError(
                f"Not enough observations after filtering/dropping NaNs for "
        ...
        best_order, best_seasonal_order, best_aic = select_best_sarma_order(
            train_series=train_t.iloc[:, 0],
            seasonal_period=7,
        )

        valid_predictions_t = rolling_one_step_forecast_sarma(
        ...
        test_predictions_t = rolling_one_step_forecast_sarma(
        ...
        LOGGER.info(
            "Completed product=%s, sign=%s, price_type=%s, order=%s, seasonal_order=%s, aic=%.4f",
            product,
            product_sign,
            price_type,
            best_order,
...
def main() -> None:
    ...
    output_folder = BASE_OUTPUT_PATH / "sarma_model_output"
    ...
    overview_df.to_csv(output_folder / "sarma_models_overview.csv", index=False)

    n_success = int(overview_df["SUCCESS"].sum()) if not overview_df.empty else 0
    LOGGER.info("Finished SARMA runs: %s/%s successful", n_success, len(overview_df))


# Example usage
# python3 -m statistical_methods.sarma
